...
Ext.define('portal.layer.querier.wfs.WFSWithMapQuerier', {
    ...
    _handleWMSQuery : function(queryTarget,callback,scope){
      //VT:app-schema wms requires the gml version to be declared in the info_format
        ...
        console.log("wmsOnlineResource", wmsOnlineResource);
        if (queryTarget.get('layer').get('renderer').sld_body || (typeof wmsOnlineResource != 'undefined' && wmsOnlineResource.get('url').toUpperCase().indexOf("MAPSERVER/WMSSERVER") < 0)) {
